(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('lodash'), require('qs')) :
  typeof define === 'function' && define.amd ? define(['lodash', 'qs'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.httpZ = factory(global._, global.qs));
}(this, (function (_, qs) { 'use strict';
  ...
  exports.generatePath = (path, params) => {
    ...
    return path + '?' + qs__default['default'].stringify(paramsObj, { indices: false })
  };

  exports.convertParamsArrayToObject = (params) => {
    validators.validateArray(params, 'params');

    return ___default['default'].reduce(params, (result, { name, value }, index) => {
      validators.validateNotEmptyString(name, 'param name', `param index: ${index}`);
      if (___default['default'].has(result, name)) {
        if (___default['default'].isArray(result[name])) {
  ...
  class HttpZBaseParser {
    ...
    _parseUrlencodedBody() {
      let params = qs__default['default'].parse(this.bodyRows);
      this.body.params = ___default['default'].map(params, (value, name) => ({ name, value }));
    }
  ...
  var parsers = (plainMessage) => {
    if (___default['default'].isNil(plainMessage)) {
      throw error.get('plainMessage is required')
    }
    if (!___default['default'].isString(plainMessage)) {
      throw error.get('plainMessage must be a string')
    }

    let firstRow = ___default['default'].chain(plainMessage).split(consts.EOL).head().value();
    if (consts.regexps.requestStartRow.test(firstRow)) {
      return request.parse(plainMessage)
    }
    if (consts.regexps.responseStartRow.test(firstRow)) {
      return response.parse(plainMessage)
    }
    throw error.get('Unknown plainMessage format')
  };
  ...
  class HttpZBaseBuilder {
    ...
    _generateUrlencodedBody() {
      ...
      return qs__default['default'].stringify(params, { indices: false })
    }
  ...
  var builders = (messageModel) => {
    ...
    throw error.get('Unknown messageModel format')
  };
